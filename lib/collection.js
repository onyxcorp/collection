...
Collection = function (models, options) {
    ...
    // collection current ammount of changes
    this.revision = 1;
    if (models) {
...
lodash.objects.assign(Collection.prototype, {
    ...
    set: function (models, options) {
        ...
        // everytime the set function is called (add/remove/merge models)
        // the collection is changed, so we update the revision
        this.revision += 1;
...
var methods = ['forEach', 'map', 'reduce', 'reduceRight', 'reject', 'every', 'some',
    ...
    'without', 'difference', 'initial', 'rest', 'indexOf'];
